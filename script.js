...
const beachLetterCard = document.getElementById("beachLetterCard");
...
if(beachLetterBtn){
  ...
  beachLetterCard.style.left = rect.left + "px";
  beachLetterCard.style.top = rect.top + "px";
  beachLetterCard.style.width = rect.width + "px";
  beachLetterCard.style.height = rect.height + "px";
  beachLetterCard.style.opacity = "0";

  requestAnimationFrame(() => {
    const targetWidth = Math.min(window.innerWidth * 0.88, 430);
    const targetHeight = Math.min(window.innerHeight * 0.78, 680);

    beachLetterCard.style.left = (window.innerWidth - targetWidth) / 2 + "px";
    beachLetterCard.style.top = (window.innerHeight - targetHeight) / 2 + "px";
    beachLetterCard.style.width = targetWidth + "px";
    beachLetterCard.style.height = targetHeight + "px";
    beachLetterCard.style.opacity = "1";
  });
});
}

function closeBeachLetterModal(){
  if(!beachLetterBtn || !beachLetterCard || !beachLetterModal){
    return;
  }
